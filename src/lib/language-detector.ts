...
function detectLanguageHeuristic(code: string): string {
  ...
  // Svelte patterns
  if (
    (trimmed.includes("<script>") && trimmed.includes("export let")) ||
  ...
  // Ruby patterns
  if (
    (trimmed.includes("def ") && trimmed.includes("end")) ||
  ...
  // Scala patterns
  if (
    (trimmed.includes("object ") && trimmed.includes("def main")) ||
  ...
  // MATLAB patterns
  if (
    (trimmed.includes("function ") && trimmed.includes("end")) ||
  ...
  // Solidity patterns
  if (
    trimmed.includes("pragma solidity") ||
...
/**
 * Get confidence score for detected language (0-1)
 */
export function getDetectionConfidence(
  code: string,
  detectedLanguage: string,
): number {
  if (!code || !detectedLanguage) return 0;

  const trimmed = code.trim().toLowerCase();
  const lang = detectedLanguage.toLowerCase();

  // High confidence patterns for various languages
  const highConfidencePatterns: Record<string, RegExp[]> = {
    ...
    css: [/\.[a-z-]+\s*{/, /#[a-z-]+\s*{/, /color\s*:/, /background\s*:/],
    go: [/package\s+main/, /func\s+main$$$$/, /fmt\.Print/, /import\s*\(/],
    rust: [/fn\s+main$$$$/, /println!/, /let\s+mut/, /impl\s+/],
    php: [/<\?php/, /\$\w+/, /echo\s+/, /function\s+\w+\(/],
    ruby: [/def\s+\w+/, /puts\s+/, /require\s+/, /\.each/],
    swift: [/func\s+\w+/, /var\s+\w+/, /let\s+\w+/, /import\s+Swift/],
    ...
    scala: [/object\s+\w+/, /def\s+main/, /case\s+class/, /trait\s+/],
    haskell: [/^\w+\s*::\s*/, /where/, /data\s+/, /instance\s+/],
    clojure: [/\(defn\s+/, /\(def\s+/, /\(ns\s+/],
    elixir: [/defmodule\s+/, /def\s+\w+\s+do/, /\|>/],
    r: [/<-\s*/, /library\(/, /data\.frame/, /ggplot/],
    matlab: [/function\s+.*\s+end/, /fprintf/, /disp\(/],
    julia: [/function\s+.*\s+end/, /using\s+/, /println\(/],
    perl: [/use\s+strict/, /my\s+\$/, /=~/],
    lua: [/function\s+.*\s+end/, /local\s+/, /require\(/],
    solidity: [/pragma\s+solidity/, /contract\s+/, /mapping\s*\(/],
    graphql: [/query\s+/, /mutation\s+/, /type\s+\w+\s*\{/],
    sql: [/select\s+.*\s+from/i, /insert\s+into/i, /create\s+table/i],
    bash: [/#!\/bin\/bash/, /echo\s+/, /export\s+/],
    powershell: [/write-host/i, /get-\w+/i, /\$\w+/],
    dockerfile: [/from\s+/i, /run\s+/i, /copy\s+/i],
    yaml: [/^[\w-]+:\s*/, /---/, /^\s*-\s+/],
    json: [/^\s*{/, /^\s*\[/, /"[\w-]+"\s*:/],
    xml: [/<\?xml/, /xmlns\s*=/, /<\/\w+>/],
    markdown: [/^#\s+/, /```/, /\*\*.*\*\*/],
    latex: [/\\documentclass/, /\\begin\{/, /\\usepackage/],
    assembly: [/^\s*\w+:/, /\b(mov|add|sub)\b/i],
  };

  const patterns = highConfidencePatterns[lang] || [];
  if (patterns.length === 0) return 0.5; // Default confidence for unmapped languages

  const matches = patterns.filter((pattern) => pattern.test(trimmed)).length;
